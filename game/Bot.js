	...
	Bot.prototype.startGame = function() {
		...
		let insertF = function() {
			...
			if( self.notSuccessfulF.length == 3 ) {
				self.stopGame();
				Handler.game.selectAct( { typeAct: Consts.TYPE_ACT_ENEMY_LOST } );
				//alert("Бот проиграл");
			}
			//};			
			self.numInpF++;
		};
		this.timerInF = setInterval( insertF, 3000 );
	};
	
	Bot.prototype.stopGame = function() {
		this.startedGame = false;
		clearInterval( this.timerInF );
	};
